...
my $vendor_insert_sth = $dbh->prepare("INSERT INTO aqbooksellers
                                    (id, name, currency, accountnumber, active, listprice, invoiceprice, notes)
                                    VALUES (?,?,?,?,?,?,?,?)");

sub migrate_vendor($s) {
    $vendor_insert_sth->execute($s->{id}, $s->{name}, $s->{currency}, $s->{accountnumber}, 1, $s->{currency}, $s->{currency}, $s->{notes}) or die "INSERT:ing Vendor failed: ".$vendor_insert_sth->errstr();
